...
- Reads and displays **live battery %** by polling the controller's own USB HID input report while plugged in (interval adjustable in Settings — 1/5/15/30 min, default 5), and separately via Android's own input-device battery tracking while running wirelessly (Android 12+ only - see the note below)
...
- **Battery level**: byte 30 of the standard 49-byte input report. `0xee`/`0xef` = charging/full; otherwise a 0–5 index into `{0, 1, 25, 50, 75, 100}` (not a raw percentage). Confirmed at this same offset independently in DsHidMini's own `DS3_RAW_INPUT_REPORT` struct.
